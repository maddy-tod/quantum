...
def entangle():
    ...
    print("Your qubit is currently in the state " + numToDir(aQubit) + " and the state you want to send is " + numToDir(sendDir))
    ...
    print("****** entangling *******")
    print("After the entangling process your qubit is in the state " + numToDir(aQubit) + " and the qubit which stored the direction to be transimitted is in the state " + numToDir(sendDir))
    sendDir = next(sendDir)
    print("We can see that the state to be transmitted is now +" + numToDir(sendDir) + " so it has been corrupted - so we have lost our copy :(")
    print("From this we can tell Bob's qubit must be in the state " + numToDir(flip(aQubit)))


def classicalTransmission():
    global sendDir
    global bQubit
    print("We now need to tell Bob how to change his qubit to make it match the state we wanted to send - " + numToDir(sendDir))
    print("This part of the transportation must be done along classic means of communication which is why we can't have faster than light communication ")


    bQubit = flip(aQubit)

    print("To tell Bob enter a sequence of flips that will convert Bob's state (" + numToDir(bQubit) + ") into the state we wanted to send")

    seq = input("Use F to indicate a flip and N to indicate a rotation by 1 place clockwise")

    while not re.match("^[FN]+$", seq):
...
def bobResponse():
    ...
    correct = input(b + "Is that what you were trying to send me?! (Y/N)")
